...
interface RegistrationStepsIndicatorProps {
...
const STEPS: StepInfo[] = [
  ...
  { id: ClientRegistrationStep.SERVICES, label: 'Services' },
  { id: ClientRegistrationStep.OPERATION, label: 'Opérations' },
  { id: ClientRegistrationStep.PEP_STATUS, label: 'PPE' },
  { id: ClientRegistrationStep.LBC_FT_CLASSIFICATION, label: 'LBC/FT' },
  { id: ClientRegistrationStep.FUND_ORIGIN, label: 'Origine fonds' },
  { id: ClientRegistrationStep.SUMMARY, label: 'Résumé' },
];

export function RegistrationStepsIndicator({
  currentStep,
  steps,
  onStepClick,
}: RegistrationStepsIndicatorProps) {
  return (
          ...
          <div key={step.id} className="flex items-center flex-none">
            ...
            <span
              ...
              className={cn(
                ...
                // Ajouter une classe pour indiquer que c'est cliquable
                !isLocked && 'cursor-pointer hover:text-brand-blue'
              )}
            >
              {step.label}
            </span>
